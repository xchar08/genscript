...
EOF
  ...
  e|E )
    ...
    # Update /etc/crypttab
    echo "swap      /dev/<device>    /dev/urandom   swap,cipher=aes-xts-plain64,size=8192" | sudo tee -a /etc/crypttab > /dev/null
    ...
    echo "swap_crypt UUID=$(blkid -s UUID -o value /dev/<device>) none luks,keyscript=decrypt_keyctl" | sudo tee
/etc/conf.d/dmcrypt_swap_passphrase > /dev/null
